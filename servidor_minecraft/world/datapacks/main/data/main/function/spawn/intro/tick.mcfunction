effect give @a[tag=intro] slow_falling infinite 1 true
scoreboard players add @a[tag=intro] introTimer 1

tag @a[scores={introTimer=1}] remove elytraFirework
clear @a[scores={introTimer=1}] *[minecraft:custom_data~{"hielkemaps:item":true},!minecraft:custom_data~{"tm:item":true}]
